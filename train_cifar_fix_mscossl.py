...
def main():
    ...
    def create_model(ema=False, clf_bias=True):
        model = models.WRN(2, num_class, args.matrix, classifier_bias=clf_bias)
        model = model.cuda()
    ...
    # If Matrix is enabled a custom output layer will be created with a matrix multiplication
    if args.matrix:
        teacher_head = models.MatrixOutputLayer(model.output.in_features, num_class - 1, bias=True).cuda()
        ema_teacher = models.MatrixOutputLayer(model.output.in_features, num_class - 1, bias=True).cuda()
    else:
    ...
    init_teacher, init_ema_teacher = classifier_warmup(copy.deepcopy(ema_model), train_labeled_set, train_unlabeled_set,
                                                       N_SAMPLES_PER_CLASS, num_class, use_cuda, args, args.matrix,
                                                       models)
    ...
    metrics_file = f"{args.out}/metrics_final_stage.txt"
    ...
    for epoch in range(args.epochs):
        ...
        *train_info, = train(labeled_trainloader, unlabeled_trainloader, model, ema_model, optimizer, ema_optimizer,
        ...
        # Evaluation part
        test_loss, test_acc, cm, overall_accuracy, class_wise_accuracy, f1_macro, *_ =\
            validate_teacher(test_loader, ema_model, ema_teacher, criterion, use_cuda, 'Test')
...
def train(labeled_trainloader, unlabeled_trainloader, model, ema_model, optimizer, ema_optimizer,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        crt_targets_x = torch.zeros(batch_size, num_class).scatter_(1, crt_targets_x.view(-1, 1).long(), 1)
        if use_cuda:
        ...
        with torch.no_grad():
            ...
            p_hat = torch.zeros(batch_size, num_class).cuda().scatter_(1, p_hat.view(-1, 1).long(), 1)
...
def validate_teacher(valloader, model, head, criterion, use_cuda, mode):
    ...
    return (losses.avg, top1.avg, cm, overall_accuracy, class_wise_accuracy, f1_macro, section_acc.cpu().numpy(), GM)

...
def get_cifar100(root, l_samples, u_samples, transform_train=transform_train, transform_strong=transform_strong,
    ...
    train_labeled_idxs, train_unlabeled_idxs = train_split(base_dataset.targets, l_samples, u_samples, 100, seed)

    train_labeled_dataset = CIFAR100_custom(root, train_labeled_idxs, train=True, transform=transform_train)
    train_unlabeled_dataset = CIFAR100_custom(root, train_unlabeled_idxs, train=True,
                                                transform=TransformTwice(transform_train, transform_strong))
    test_dataset = CIFAR100_custom(root, train=False, transform=transform_val, download=False)

    print(f"#Labeled: {len(train_labeled_idxs)} #Unlabeled: {len(train_unlabeled_idxs)}")
    if return_strong_labeled_set:
        train_strong_labeled_dataset = CIFAR100_custom(root, train_labeled_idxs, train=True, transform=transform_strong)
...
class CIFAR100_custom(torchvision.datasets.CIFAR100):

    def __init__(self, root, indexs=None, train=True,
                 transform=None, target_transform=None,
                 download=False):
        root = './dataset/cifar100'
        super(CIFAR100_custom, self).__init__(root, train=train,

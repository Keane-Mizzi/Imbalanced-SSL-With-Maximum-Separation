...
def main():
    ...
    print('Mean bAcc:')
    print(test_accs[-20:])
    print(np.mean([t.item() if isinstance(t, torch.Tensor) else t for t in test_accs[-20:]]))  # Use .item() to get float value from CUDA tensor, handle scalar float values

    print('Mean GM:')
    print(np.mean([t.item() if isinstance(t, torch.Tensor) else t for t in test_gms[-20:]]))  # Use .item() to get float value from CUDA tensor, handle scalar float values
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            if select_mask.sum() != 0:
                ...
                select_mask = select_mask.to(total_acc.device)
        ...
        select_mask = select_mask.to('cuda')

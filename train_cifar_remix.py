...
from utils import make_imb_data, save_checkpoint, ReMixMatch_Loss, WeightEMA, linear_rampup, interleave
...
# Matrix
parser.add_argument('--matrix', type=bool, default=False, help='include matrix multiplication')
...
def main():
    ...
    def create_model(ema=False):
        model = models.WRN(2, num_class, args.matrix)
        model = model.cuda()
    ...
    train_criterion = ReMixMatch_Loss(args.lambda_u, args.epochs)
    ...
    if args.resume:
        ...
        emp_distb_u = checkpoint['emp_distb_u']
    else:
    ...
    # Creating an empty metrics file
    metrics_file = f"{args.out}/metrics_first_stage.txt"

    intro_metrics = f"Matrix: {args.matrix}\n" \
        f"Class labels: {train_labeled_set.classes}\n"\
        f"Labelled set: {N_SAMPLES_PER_CLASS}\n"\
        f"Unlabelled set: {U_SAMPLES_PER_CLASS}\n"\
        f"{'-' * 80}\n"

    with open(metrics_file, 'w') as f:
        f.write(intro_metrics)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Note: for this implementation, whether normalize target_disb leads to equivalent result!!!

        # Training part
        train_loss, train_loss_x, train_loss_u, total_c, emp_distb_u = train(labeled_trainloader,
        ...
        # Evaluation part
        test_loss, test_acc, test_cls, test_gm, overall_accuracy, class_wise_accuracy, f1_macro, cm =\
            validate(test_loader, ema_model, criterion, use_cuda, mode='Test Stats ')
        ...
        class_wise_accuracies.append(class_wise_accuracy)
        ...
        save_checkpoint({
            ...
            'emp_distb_u': emp_distb_u,
        }, epoch, args.out)
        test_accs.append(test_acc)
        test_gms.append(test_gm)

        if epoch == 0 or epoch % 50 == 0 or epoch == args.epochs - 1:
            formatted_metrics = f"Current Epoch: {epoch + 1}\n" \
                                f"Overall Accuracy: {overall_accuracies}\n" \
                                f"Class-wise Accuracies: {[arr.tolist() for arr in class_wise_accuracies]}\n" \
                                f"Macro F1-scores: {macro_f1_scores}\n" \
                                f"{'-' * 80}\n"
    ...
    with open(metrics_file, 'a') as f:
        ...
        # Write column labels
        f.write(" " * 10)
        for label in train_labeled_set.classes[:num_class]:
            f.write(f"{label: <12}")
        f.write("\n")

        # Write row labels and values
        for i, row_label in enumerate(train_labeled_set.classes[:num_class]):
            f.write(f"{row_label: <10}")
            for value in cm[i, :num_class]:
                f.write(f"{value: <12}")
            f.write("\n")

    print('Mean bAcc:')
    print(test_accs[-20:])
    print(np.mean([t.item() if isinstance(t, torch.Tensor) else t for t in test_accs[-20:]]))

    print('Mean GM:')
    print(np.mean([t.item() if isinstance(t, torch.Tensor) else t for t in test_gms[-20:]]))

    print("Overall Accuracies:", overall_accuracies)
    print("Class-wise Accuracies:", class_wise_accuracies)
    print("Macro-averaged F1-scores:", macro_f1_scores)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        loss = Lx + w * Lu + args.w_rot * Lr + args.w_ent * Le * linear_rampup(epoch+batch_idx/args.val_iteration, args.epochs)
...
def validate(valloader, model, criterion, use_cuda, mode):
    ...
    precision, recall, f1_macro, _ = precision_recall_fscore_support(y_true, y_pred, average='macro', zero_division=1)
    ...
    return (losses.avg, top1.avg, section_acc.cpu().numpy(), GM, overall_accuracy, class_wise_accuracy, f1_macro, cm)

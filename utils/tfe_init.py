...
def classifier_warmup(model, train_labeled_set, train_unlabeled_set, N_SAMPLES_PER_CLASS, num_class,
                      use_cuda, args, is_matrix, models):
    ...
    matrix_num_class = num_class - 1 if is_matrix else num_class
    tfe_model = weight_imprint(copy.deepcopy(model), train_labeled_set, matrix_num_class, is_matrix, models)
...
def TFE(labeled_loader, unlabeled_loader, tfe_model, num_classes, num_samples_per_class):
    ...
    with torch.no_grad():
        ...
        print(labeled_set_idx.device)
        print(labeled_target_stack.device)
        # labeled_set_idx = labeled_set_idx.to('cuda')
        labeled_target_stack = labeled_target_stack.to('cpu')
        ...
        labeled_target_stack = labeled_target_stack.to('cuda')
        target_stack = torch.zeros(len(labeled_target_stack), num_classes).cuda().scatter_(1, labeled_target_stack.view(-1, 1).long(), 1)
...
def weight_imprint(model, labeled_set, num_classes, is_matrix, models):
    model = model.cuda()
    model.eval()
    ...
    if is_matrix:
        model.output = models.MatrixOutputLayer(model.output.in_features, num_classes, bias=False).cuda()
    else:
...
def classifier_train(labeled_trainloader, model, optimizer, scheduler, ema_optimizer, val_iteration,
                     use_cuda):

...
import create_max_separated_matrix as sepmax


def set_num_class_global(num_class):
    global num_class_global
    num_class_global = num_class


def set_prototypes():
    global prototypes_global
    prototypes_global = torch.from_numpy(sepmax.create_prototypes(num_class_global)).to('cuda').t()

def set_matrix(matrix_status):
    global matrix
...
class WRN(nn.Module):
    ...
    def __init__(self, width, num_classes, matrix, rotation=True, block_name='residual', rotnet=None, classifier_bias=True):
        ...
        set_num_class_global(num_classes)
        set_prototypes()
        set_matrix(matrix)
        ...
        if matrix:
            self.output = MatrixOutputLayer(filters[3], num_classes-1, bias=classifier_bias)
        else:
...
class MatrixOutputLayer(nn.Linear):
    def __init__(self, in_features, out_features, bias=True):
        super().__init__(in_features, out_features, bias)

    def forward(self, input):
        output = super().forward(input)
        output = torch.mm(output, prototypes_global)
        return output